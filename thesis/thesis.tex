...
parametrycznego. Zazwyczaj nie pozwalają na przekazywanie dowolnych 
typów przez ich wartość do funkcji polimorficznych. W swojej pracy przedstawię 
implementację kompilatora języka funkcyjnego, w której unikam jednorodnej 
...
wprowadzając tym samym \textit{ad--hoc} polimorfizm. Będą one głównym elementem 
...
\englishabstract{ TODO english abstract, pozostawiam na koniec. }
...
Składnia wyrażeń warunkowych jest bardzo podobna do tej w F\#. W 
języku $Lang$
...
Poniższa gramatyka jest zbliżona do rzeczywistej składni języka.
...
Rekordy w $Langu$, podobnie jak rekordy w F\# i OCamlu, 
są trwałe. 
...
\begin{enumerate}
  ...
  \path{lang-compiler/compiler/parsing/lexer.cppo.sedlex.ml}
  ...
  \end{enumerate}
  Implementacja w pliku: 
  \path{lang-compiler/compiler/typed_ast.ml}
  ...
  Implementacja w pliku: 
  \path{lang-compiler/compiler/codegen.ml}
...
\begin{enumerate}
  ...
  \item Zaaplikowanych argumentów jest tyle samo co zdefiniowanych. 
  Funkcja zostanie wywołana w stylu z C. Jest to najbardziej optymalny 
  ...
  liczbą argumentów w definicji funkcji, a $n$ liczbą aplikowanych 
  argumentów, gdzie $n > k$. Najpierw nastąpi wywołanie znanej 
  funkcji z pierwszymi $k$ argumentami. Do wyniku pierwszego wywołania, 
  który teraz jest nieznaną funkcją, zostanie zaaplikowane pozostałe 
...
rozważenie wszystkich przypadków związanych z liczbą zaaplikowanych argumentów.
...
Powyższy kod nie jest poprawnym programem w języku OCaml, oraz 
...
% Moje rozwiązanie, choć z założenia szybsze i bardziej oszczędne w zasobach 
% niż 
