...
Zmniejszając narzut pamięciowy związany z implementacją funkcji polimorficznych,
...
Monomorfizacja funkcji pozwoli na statyczne 
wybranie odpowiednich instancji klasy. 
}
\englishabstract{Popular compilers for ML-family functional programming 
languages, often made similiar trade-offs when implementing partial aplication
and parametric polimorphism. They usually require uniform type representation 
which leads to boxing of all polymorphic arguments. In this thesis I present 
...
Definiujemy klasę $Eq$ zawierającą dwa operatory:~$==$ oraz~$/=$.
% \lstset{language=llvm}
\begin{lstlisting}[frame=single]
class Eq a where
  (==) :: a -> a -> Bool
...
Powiemy, że typ ukonkretniony z $a$ jest instancją klasy $Eq$, 
...
\begin{lstlisting}[frame=single, caption=Instancja klasy \texttt{Eq} dla typu \texttt{Bool}.]
...
Poniższy fragment kodu zawiera dwie proste funkcje wzajemnie rekurencyjne.
...
Gramatyka deklaracji rekordów znajduje się na kolejnym listingu.
...
Tablice \newline są ulotną strukturą danych. Na poziomie języka można
 stworzyć literał 
...
funkcji o \newline typie int $\rightarrow$ bool $\rightarrow$ int 
...
\begin{itemize}
  ...
  generowania \newline kodu LLVM IR w funkcyjny interfejs. Bazująca w przeważającej większości 
...
\begin{itemize}
  ...
  kodu \newline LLVM z użyciem programu \texttt{llc} (kompilator LLVM IR) i 
  \texttt{gcc}.
