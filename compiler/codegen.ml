...
and gen_if env cond then_exp else_exp =
  ...
  let then_bb = append_block env.ctx "then" the_function in

  (* Emit 'then' value. *)
  position_at_end then_bb env.builder;
  let then_val = gen_expr env then_exp |> fst in
  let new_then_bb = insertion_block env.builder in

  let else_bb = append_block env.ctx "else" the_function in
  position_at_end else_bb env.builder;

  let else_val = gen_expr env else_exp |> fst in

  let new_else_bb = insertion_block env.builder in
  let merge_bb = append_block env.ctx "if_cont" the_function in
  position_at_end merge_bb env.builder;

  let incoming = [(then_val, new_then_bb); (else_val, new_else_bb)] in
  let phi = build_phi incoming "if_tmp" env.builder in
...
and gen_letexp env (name, ret_type) args fst_line body_lines =
  ...
  let ret_val = gen_exprs inner_env bb body_exprs |> fst in
  ...
  (if ret_val |> type_of |> classify_type = TypeKind.Void
   then build_ret_void env.builder
   else build_ret ret_val env.builder)
  |> ignore;
...
and gen_exprs env let_block =
  ...
  | e::es -> position_at_end let_block env.builder;
